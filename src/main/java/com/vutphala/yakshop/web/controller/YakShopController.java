...
public class YakShopController {
	...
	public String checkOrder(@PathVariable Integer day,
			OrderRequest orderRequest, HttpServletResponse response) {
	...
	@ResponseBody
	public String stockCalendar(@RequestParam("start") String paramStartDate,@RequestParam("end") String paramEndDateValue, HttpServletResponse response) {
		...
		int startDays = Days.daysBetween(
				new DateTime(startDate.getTimeInMillis()),
		...
		try {
			...
			if (startDays > 0 && endDays > 0) {
				...
				for (int calCounter = startDays; calCounter <= endDays; calCounter++) {
					...
					evendata.put("start",
							(fmt.print(new DateTime(new Date()).plusDays(calCounter))));
					events.put(evendata);
				}
			}
		...
		/*
		 * HeardInfo info = heardService.getHeardDetailsForTheDay(day);
		 * JSONArray yaks = new JSONArray(); try { JSONObject stock = new
		 * JSONObject(); stock.put("milk", info.getMilkStock());
		 * stock.put("skins", info.getWoolStock());
		 * 
		 * response.setStatus(HttpServletResponse.SC_OK);
		 * response.setContentType("");
